...
Task selection prefers an already-`in_progress` task within the configured
scope, so an interrupted task is resumed rather than stranded. This is
necessary because `bd ready` only returns unblocked open work.
