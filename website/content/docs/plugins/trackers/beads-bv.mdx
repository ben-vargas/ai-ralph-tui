...
<Callout type="info">
`bv`'s robot flags do not apply label scoping to `--robot-next`. Ralph therefore
verifies each `bv` pick against the configured labels and falls back to `bd`'s
native label filtering when it does not match. Epic scoping remains the primary
mechanism for narrowing smart selections.
</Callout>
