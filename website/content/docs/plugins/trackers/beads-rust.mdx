...
The Beads-Rust store can live outside the working directory. Set `BEADS_DIR`
to the store path, or use an absolute `beadsDir` path. This supports shared or
...
3. Otherwise use `br ready` to select open work with no unresolved dependencies
...
2. **Tasks are `open` or `in_progress` status**: `br list --all` (tasks stuck in another status, such as `blocked`, won't be selected)
