...
| `--watch` | Keep the engine alive and start new tasks automatically as they become available |
| `--poll <seconds>` | Poll for new tasks at the given interval (default: 30 seconds); implies `--watch` |
...
9. **Repeats** until no tasks remain or max iterations reached; with `--watch`, waits and polls instead of stopping when no tasks remain
